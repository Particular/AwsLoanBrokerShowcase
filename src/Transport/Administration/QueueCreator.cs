...
class QueueCreator(AzureServiceBusTransport transportSettings)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            const int emulatorMaxDeliveryCount = 10;
            var isUsingEmulator = transportSettings.ConnectionString?.Contains("UseDevelopmentEmulator") ?? false;

            var queue = new CreateQueueOptions(address)
            {
                EnableBatchedOperations = true,
                LockDuration = TimeSpan.FromMinutes(5),
                MaxDeliveryCount = isUsingEmulator ? emulatorMaxDeliveryCount : int.MaxValue,
