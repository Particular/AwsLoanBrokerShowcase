...
public partial class AzureServiceBusTransport : TransportDefinition
{
        ...
        ReceiveSettings[] receivers, string[] sendingAddresses, CancellationToken cancellationToken = default)
    {
        ...
        ApplyRetryPolicyOptionsIfNeeded(clientOptions);
        ApplyWebProxyIfNeeded(clientOptions);

        var sharedClient = TokenCredential != null
            ? new ServiceBusClient(FullyQualifiedNamespace, TokenCredential, clientOptions)
            : new ServiceBusClient(ConnectionString, clientOptions);

        var receiveSettingsAndClientPairs = receivers.Select(receiver => (receiver, sharedClient)).ToArray();
        var defaultClient = sharedClient;
